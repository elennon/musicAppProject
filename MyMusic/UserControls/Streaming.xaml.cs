...
namespace MyMusic.Views
{
    ...
    public sealed partial class Streaming : Page
    {
        ...
        {
            string ssid = "e781e545477c5f80ae419146924d4d3f";
            var response3 = await client.GetAsync("api/GrooveShark/getTrack/?artist=" + txtArtist.Text + "&track=" + txtTrack.Text + "&sessionId=" + ssid);

            var streamUri = response3.Content.ReadAsStringAsync().Result;
            string hj = streamUri.Split('"')[1];
            string gsUrl = "gsStreamTrack," + hj;
            if (!Frame.Navigate(typeof(NowPlaying), gsUrl))
            {
