    ...
    public class StreamFinderRadioStations
    {
        ...
        private bool SaveToDB(Stations rdos)
        {
            ...
            foreach (var item in results)   // each group of genres
            {
                ...
                rg.RadioStreams = new List<RadioStream>();
                foreach (var station in item.sts)
                {
                    RadioStream rs = new RadioStream { RadioName = station.Name, StatnType = Helper_Classes.RootObject.StationType.StreamFinder.ToString() };  // add each station to its respective genre
                    if ((station.Urls.FirstOrDefault()).urlName != null)
                    {
                        rs.RadioUrl = (station.Urls.FirstOrDefault()).urlName;
                    }
                    else
                    {
                        string h = "h";
            ...
            {
                cd.insertGenre(item);
                foreach (var radio in item.RadioStreams)
                {
